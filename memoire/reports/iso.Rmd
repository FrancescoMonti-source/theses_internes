...
# Analisi descrittiva
```{r}
pat_operes = pat_operes %>% nest(pmsi = c(codeacte,label_acte,type_diag,code,label_code, ufpro)) %>% select(-datsort)

data = pat_operes %>% full_join((infections[-3]))

# 652 pazienti operati al rachide in tutto l'ospedale
gente_operata = data %$% n_distinct(patid)

#
gente_infetta = data %>% filter(infection==1) %$% n_distinct(patid)
# UF frequentate
pat_operes$ufpro %>% table
```

Durante il 2020, al CHU, `r gente operata` soggetti sono stati operati al rachide

Di questi,`r gente_infetta` soggiorneranno di nuovo presi in carico all'ospedale, per un infezione verosimilmente legata all'atto chirurgico, nei 13 mesi successivi.
